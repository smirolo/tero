...
int main( int argc, char *argv[] )
{
	...
	pw = getpwuid(getuid());
	grp = getgrgid(getgid());
	cerr << "real      : " << getuid() << '(' << pw->pw_name << ")\t" << getgid() << '(' << grp->gr_name << ')' << endl;

	pw = getpwuid(geteuid());
	grp = getgrgid(getegid());
    cerr << "effective : " << geteuid() << '(' << pw->pw_name << ")\t" << getegid() << '(' << grp->gr_name << ')' << endl;
    ...
    try {
		...
		if( s.vars["view"].empty() ) {
			...
			std::cerr << "docAsUrl:" << s.docAsUrl() << std::endl;
			...
			cppLight leftCppStrm(leftLinkStrm);
			cppLight rightCppStrm(rightLinkStrm);
			text cpp(leftLinkStrm,rightLinkStrm);
			...
			gitcmd revision(s.valueOf("binDir") + "/git",s.valueOf("topSrc"));
