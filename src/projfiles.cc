...
void projfiles::fetch( session& s, const boost::filesystem::path& pathname ) 
{
	...
	std::string topSrc = s.vars.find("topSrc")->second;
	bool foundProject = exists(dirname.string() + "/index.xml");
	while( !foundProject & dirname.string() != topSrc ) {
		std::cerr << "look for index.xml into " << dirname << std::endl;
		dirname.remove_leaf();
		if( dirname.string().empty() ) {
			boost::throw_exception(basic_filesystem_error<path>(std::string("no index.xml from path up"),
																pathname, 
																error_code())); 
		}
		foundProject = exists(dirname.string() + "/index.xml");
	}
	
	if( foundProject ) {
		/* We insert pathnames into a set<> such that they later can be iterated 
